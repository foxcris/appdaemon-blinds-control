            ...
            if cooldownnightdown < datetime.now():
                ...
                if (self.get_state(
                    ...
                    self._log(
                        ...
                        cooldownnightdown<self.time_open_blinds.\
                        check again in 5 minutes",
                        prefix=entityid)
                    self._log("close_blinds_cooldown: cooldownnightdown: %s" %
                              cooldownnightdown, prefix=entityid)
                    self._log("close_blinds_cooldown:\
                              self.time_open_blinds: %s" %
                              self._get_variable(
                                  entityid, "time_open_blinds"),
                              prefix=entityid)
                    self._set_handle(entityid, "cbcd_handle", self.run_at(
                    ...
                    self._log("close_blinds_cooldown: Trigger\
                              cooldownnightdown: %s" %
                              cooldownnightdown,
                              prefix=entityid)
                    self._set_handle(entityid, "cbcd_handle", self.run_at(
            ...
            if cooldownnightup < datetime.now():
                ...
                if (self.get_state(
                    ...
                    self._log(
                        ...
                        self.time_close_blinds. check again in 5 minutes",
                        prefix=entityid)
                    self._log("open_blinds_cooldown: cooldownnightup: %s" %
                              cooldownnightup, prefix=entityid)
                    self._log(
                        "open_blinds_cooldown: self.time_close_blinds: %s" %
                        self._get_variable(entityid, "timeclose_blinds"),
                        prefix=entityid)
                    self._set_handle(entityid, "obcd_handle", self.run_at(
                        self._open_blinds_cooldown,
                        datetime.now() + timedelta(minutes=5),
                    ...
                    self._log("open_blinds_cooldown: Trigger cooldownnightup:\
